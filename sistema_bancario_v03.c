...
void Login(PGconn *conn, int *ID) {
    ...
    char sql[300];
    snprintf(sql, sizeof(sql), "SELECT * FROM sistema where usuario='%s' AND senha=crypt('%s', senha);", usuario, senha);
...
bool Verificar_senha(PGconn *conn, int ID) {
    char senha[25], sql[300];

    printf("\n Digite sua senha: ");
    scanf("%s", senha);

    snprintf(sql, sizeof(sql),
    "SELECT * FROM sistema where id=%d AND senha=crypt('%s', senha);",ID,senha
    );
    PGresult *res = PQexec(conn, sql);
